...
5. **Claude Code** — rewrites `~/.claude/themes/labstick.json`.  Claude Code draws its own input box, dim hints and diff highlighting, and defaults to `dark`, which is unreadable on a light terminal.  The `theme` setting is pointed once at `custom:labstick` and then left alone; the theme file carries the mode in its `base` field.  Claude Code watches its themes directory, so a running session repaints, which flipping the setting itself does not achieve.  A theme's slug is its filename, hence `themes/labstick.json` → `custom:labstick`.
